...
export class BillingController {
  ...
  async getClientData(@Param('clientId') clientId: string) {
    try {
      const response = await firstValueFrom(this.client.send<string, string>('getClientData', clientId));
      return response;
    } catch (error) {
  ...
  async getProductData(@Param('sku') sku: string) {
    try {
      const response = await firstValueFrom(this.client.send<string, string>('getProductData', sku));
      return response;
    } catch (error) {
      console.error('Error communicating with the microservice:', error.message);
  ...
  async createOrder(@Body() createOrderDto: CreateBillingDto) {
    try {
      const response = await firstValueFrom(this.client.send('createOrder', createOrderDto));
      return response;
    } catch (error) {
      console.error('Error communicating with the microservice:', error.message);
